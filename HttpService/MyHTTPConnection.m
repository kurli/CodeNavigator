...
{    
    ...
    if ([returnPath length] == 0)
        returnPath = @"as:   Project";
    else
        returnPath = [NSString stringWithFormat:@"to:   %@", returnPath];
    NSMutableString *outdata = [NSMutableString new];
	[outdata appendString:@"<html><head>"];
	[outdata appendFormat:@"<title>CodeNavigator Web Upload Service</title>"];
	...
	[outdata appendFormat:@"<h1>CodeNavigator Web Upload Service</hi><br><br><h3>Files Upload %@</h3>", returnPath];
    [outdata appendString:@"<br><br>"];
...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
	...
	if (requestContentLength > 0)  // Process POST data
	{
		...
		{
            [self.webUploadResultDelegate startUploadingFile:filename];
            ...
            [self.webUploadResultDelegate finishUploadingFile:filename];
