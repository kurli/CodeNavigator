				   ...
				   delegate:(id<FPPopoverControllerDelegate>)delegate
{
    ...
    if(self)
    {
        ...
        self.popoverContentSize = CGSizeMake(200, 300); //default size

        _contentView = [[FPPopoverView alloc] initWithFrame:CGRectMake(0, 0, 
                                              self.popoverContentSize.width, self.popoverContentSize.height)];
...
- (void)presentPopoverFromRect:(CGRect)rect inView:(UIView *)view permittedArrowDirections:(UIPopoverArrowDirection)arrowDirections animated:(BOOL)animated {
    [self presentPopoverFromPoint:rect.origin];
}
...
-(CGRect)bestArrowDirectionAndFrameFromView:(UIView*)v
{
    ...
    CGRect r;
    r.size = self.popoverContentSize;
    if (r.size.height > [self parentHeight]/3*2) {
        r.size.height = [self parentHeight]/3*2;
    }
