...
-(BOOL) checkPreprocessor: (int) lineNumber
{
    ...
    // check @ preprocessor first
    NSRange atPreprocessor = [needParseLine rangeOfString:@"@"];
    if (atPreprocessor.location == 0) {
        NSRange strRange = [needParseLine rangeOfString:@"\""];
        if (strRange.location == 1) {
            return NO;
        }
	...
	NSRange headerBegin = [needParseLine rangeOfString: PREPROCESSOR_HEADER];
...
// return YES, if we need to restart parse
// return NO, we do not need to reparse
-(BOOL) checkATBeginner: (int) lineNumber
{
	...
	NSRange ATBegin = [needParseLine rangeOfString: @"@"];
	if ( ATBegin.location != 0 )
		return NO;
    
	NSRange strbegin = [needParseLine rangeOfString: @"\""];
	if ( strbegin.location == 1 )
		return NO;
...
-(BOOL) checkOthers: (int) lineNumber
{
    ...
    {
        // for start with NS or CF or CG or UI
        if ([word rangeOfString:@"NS"].location == 0 ||
            [word rangeOfString:@"CF"].location == 0 ||
            [word rangeOfString:@"CG"].location == 0 ||
            [word rangeOfString:@"UI"].location == 0)
        {
            [self systemStart];
            [self addString:word addEnter:NO];
            [self addEnd];
