...
-(id) init
{
    [self setParserConfigName:@"Go"];
	if ( (self = [super init])!=nil )
	{
        //****
//        NSString* keywords = @"break case chan const continue default defer else fallthrough for func go goto if import interface map package range return select struct switch type var bool byte complex64 complex128 float32 float64 int8 int16 int32 int64 string uint8 uint16 uint32 uint64 int uint uintptr true false iota nil append cap close complex copy imag len make new panic print println real recover";
